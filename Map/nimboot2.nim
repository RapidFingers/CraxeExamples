...
converter toInt*(v:int):Int =
    return Int(value: v)

converter toint*(v:Int):int =
    if not v.isNil():
        return v.value
    raise newException(NilAccessError,"")
...
template get*[K, Int](this:HaxeMap[K, Int], key:K):Int =
    this.data.getOrDefault(key, nil)
